  ...
  return (
    <section id="projects" className="py-20">
      <div className="max-w-6xl mx-auto px-4">
        <h2 className="text-3xl font-bold text-center mb-12 text-[#1F2937]">Projects</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project) => (
            <div key={project.title} className="bg-white rounded-lg overflow-hidden shadow-sm border border-[#E5E7EB] transform transition-transform duration-300 hover:scale-[1.01] hover:shadow-md">
              <img
                src={project.image}
                alt={project.title}
              ...
              />
              <div className="p-6">
                <h3 className="text-xl font-semibold text-[#374151] mb-2">{project.title}</h3>
                <p className="text-[#4B5563] mb-4">{project.description}</p>
                <a
                  href={project.link}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-block bg-[#16A34A] hover:bg-[#15803D] !text-white font-semibold py-2 px-4 rounded-md transition-colors"
                >
                  View on GitHub
                </a>
              </div>
